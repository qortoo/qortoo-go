...
Every `Client`, datatype (`Counter`, `Variable`), and `LocalConnectivity` owns a native
handle. Call `Close` deterministically; the runtime cleanup registered by each
constructor is only a leak fallback and is not guaranteed to run, including at process
exit.

## Ownership and Close Order

- A datatype keeps its owning `Client` reachable until the datatype is closed.
- Closing a `Client` shuts down its worker runtime. Existing datatypes remain safe to
...
For normal application shutdown, close datatypes before clients and close the shared
...
```text
datatype.Close -> Client.Close -> LocalConnectivity.Close
...
goroutine. Its `tx` datatype handle is borrowed from Rust and is valid only for the
duration of that function. Do not retain it, pass it to another goroutine, or call
`Close` on it. Returning an error or panicking rolls back every operation performed
through `tx`; a Go panic is recovered before it can unwind across the FFI boundary.

Rolling back restores the state each operation replaced, not an approximation of it: an
aborted `Variable` transaction leaves the value and the timestamp the variable held
before the transaction started, however many `Set` calls it made.
